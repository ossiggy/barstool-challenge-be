...
describe('mlbController', () => {
  ...
  it('Should create a game on POST', () => {
    ...
    return agent
      ...
      .send(testData)
      .then(res => {
        const hasKeys = gameFields.reduce((acc, x) => acc && res.body.hasOwnProperty(x));
  ...
  it('Should send back a game by id', () => {
    return createMockGame().then(game => {
      let agent = chai.request.agent(app);
      return agent.get(`/mlb/${game._id}`).then(res => {
        const hasKeys = gameFields.reduce((acc, x) => acc && res.body.hasOwnProperty(x));
    ...
    return createMockGame().then(game => {
      ...
      return agent
        ...
        .then(res => {
          ...
          assert.equal(res.body.officials[0].position, mockPayload.officials[0].position);
          assert.equal(res.body.officials[0].last_name, mockPayload.officials[0].last_name);
          assert.equal(res.body.officials[0].first_name, mockPayload.officials[0].first_name);
          return res;
        });
    });
  });
